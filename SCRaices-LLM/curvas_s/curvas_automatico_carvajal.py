"""
curvas_automatico_carvajal.py  -  Automatizacion semanal Curvas S - Proyecto P128 Com. José Carvajal
=====================================================================================================
- Fecha de control:  date.today() (automatica)
- Datos de avance:   leidos desde AppSheet (proyecto P128)
  Fallback:          hoja 'Datos Control' del spreadsheet Gantt Control Comunidad José Carvajal
- Genera 5 graficos PNG (3 grupos + total + todos grupos)
- Actualiza Drive, marca PENDIENTE para Apps Script

Ejecucion manual:  python curvas_automatico_carvajal.py
"""
...
# Curva S programada semanal Carvajal (36 semanas = 252 dias)
# Linear: 37 puntos alcanzando 100% en 36 semanas
...
CHART_NAMES = [
    "CurvaS_GRUPO_1_Carvajal.png",
    "CurvaS_GRUPO_2_Carvajal.png",
    "CurvaS_GRUPO_3_Carvajal.png",
    "CurvaS_TOTAL_Carvajal.png",
    "CurvaS_Todos_Grupos_Carvajal.png",
]

BENEFICIARIOS_SETUP = [
    # GRUPO 1
    ("GRUPO 1", "Alexia Yazmin Carvajal Aillapán",         "16/11/2026"),
    ("GRUPO 1", "Gloria Angélica Córdova Mellao",           "16/11/2026"),
    ("GRUPO 1", "Luis Diego Carvajal Zeballos",             "23/11/2026"),
    # GRUPO 2
    ("GRUPO 2", "Eduardo César Carvajal Paillalef",         "23/11/2026"),
    ("GRUPO 2", "Georgina Del Carmen Reyes Carvajal",       "30/11/2026"),
    ("GRUPO 2", "Gonzalo Humberto Sánchez Reyes",           "30/11/2026"),
    ("GRUPO 2", "Isaías Bernabé Reyes Carvajal",            "07/12/2026"),
    ("GRUPO 2", "Johanna Claribel Carvajal Cañupan",        "07/12/2026"),
    ("GRUPO 2", "Luzmira Reyes Carrillo",                   "14/12/2026"),
    # GRUPO 3
    ("GRUPO 3", "Herminia Del Carmen Mellao Pacheco",        "14/12/2026"),
    ("GRUPO 3", "Marta Elena Carvajal Painehual",           "21/12/2026"),
    ("GRUPO 3", "Víctor Manuel Carvajal Painehual",         "21/12/2026"),
]

# ─────────────────────────────────────────────────────────────────────────────
# LOG
# ─────────────────────────────────────────────────────────────────────────────
log = _ccu.setup_logging("carvajal")
...
def _load_drive_ids():
    return _ccu.load_drive_ids("jose_carvajal", {})


def _save_drive_ids(ids):
    _ccu.save_drive_ids("jose_carvajal", ids)
...
def sincronizar_grupos_desde_gantt(sheets_svc):
    ...
    log.info("Verificando movimientos de grupo en 'Programa de obra' (Carvajal)...")

    r = sheets_svc.spreadsheets().values().get(
        spreadsheetId=SPREADSHEET_ID,
        range="'Programa de obra'!A1:L30",
...
def leer_avance_appsheet():
    if not APPSHEET_DISPONIBLE:
        ...
        return None
    try:
        log.info(f"Leyendo avance desde AppSheet {APPSHEET_PROJECT} (Carvajal)...")
...
def leer_datos_control(sheets_svc):
    """Lee 'Datos Control' de Carvajal. % real = AppSheet P128 (o col D como fallback)."""
    ...
    for row in rows[4:]:
        ...
        if not grupo_raw or not nombre:
            continue
        # Saltar filas de cabecera de grupo
...
def generar_grafico_grupo(nombre_grupo, beneficiarios, control, outdir, pct_prog_gantt=None):
    ...
    ax.set_title(
        f"Curva S - {nombre_grupo} - Com. José Carvajal P128 ({len(beneficiarios)} viviendas)\n"
    ...
    fname = Path(outdir) / f"CurvaS_{slug}_Carvajal.png"
...
def generar_grafico_total(grupos, control, fines_proy_global, outdir, pct_prog_gantt_total=None):
    ...
    ax2.set_title(
        f"Curva S Total - Com. José Carvajal P128 ({len(todos)} viviendas)\n"
    ...
    plt.tight_layout()
    fname_total = Path(outdir) / "CurvaS_TOTAL_Carvajal.png"
...
def generar_grafico_todos(grupos, control, fines_proy_global, outdir):
    ...
    ax3.set_title(
        "Curva S por Grupo - Com. José Carvajal P128\n"
    ...
    plt.tight_layout()
    fname = Path(outdir) / "CurvaS_Todos_Grupos_Carvajal.png"
    plt.savefig(fname, dpi=DPI_EXPORT, bbox_inches="tight"); plt.close()
    log.info(f"  Grafico guardado: {fname.name}")


# ─────────────────────────────────────────────────────────────────────────────
# 5. INSERTAR IMAGENES EN HOJA 'Curva S'
# ─────────────────────────────────────────────────────────────────────────────
def insertar_imagenes_en_sheets(sheets_svc, drive_ids):
    """Actualiza formulas IMAGE() en hoja 'Curva S': 2 cols x 3 filas."""
    log.info("Actualizando formulas IMAGE() en hoja 'Curva S' Carvajal...")

    meta = sheets_svc.spreadsheets().get(spreadsheetId=SPREADSHEET_ID).execute()
    cruvas_gid = None
    for s in meta["sheets"]:
        if s["properties"]["title"] == "Curva S":
            cruvas_gid = s["properties"]["sheetId"]
            break
    if cruvas_gid is None:
        log.error("Hoja 'Curva S' no encontrada.")
        return

    W_PX, H_PX = 755, 385
    layout = [
        ("CurvaS_GRUPO_1_Carvajal.png",      0, 0),
        ("CurvaS_GRUPO_2_Carvajal.png",       1, 0),
        ("CurvaS_GRUPO_3_Carvajal.png",       0, 1),
        ("CurvaS_TOTAL_Carvajal.png",         1, 1),
        ("CurvaS_Todos_Grupos_Carvajal.png",  0, 2),
    ]
    ...
    requests = [
        {"updateDimensionProperties": {
            "range": {"sheetId": cruvas_gid, "dimension": "COLUMNS",
                      "startIndex": 0, "endIndex": n_cols},
            "properties": {"pixelSize": W_PX}, "fields": "pixelSize"
        }},
        {"updateDimensionProperties": {
            "range": {"sheetId": cruvas_gid, "dimension": "ROWS",
...
def actualizar_pct_en_hoja(sheets_svc, grupos):
    """Escribe % real en 'Datos Control' col D. OBLIGATORIO: round(pct, 2)."""
    valores = []
...
def main():
    ...
    log.info("=" * 65)
    log.info(f"INICIO CURVAS S COM. JOSE CARVAJAL P128  -  {ts}")
    log.info("=" * 65)
    ...
    try:
        ...
        log.info("Generando graficos Com. José Carvajal P128...")
        fines_proy_global = []
        for nombre_grupo, beneficiarios in grupos.items():
