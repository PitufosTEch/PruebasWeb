...
- Datos de avance: AppSheet P28 (fallback: hoja 'Datos Control')
- 1 grupo · 1 beneficiaria
- Genera 3 PNGs: GRUPO 1 + total + todos grupos
- Actualiza Google Drive y hoja 'Curvas S' en el Gantt de control

Estructura de columnas en 'Programa de obra':
- idx 0: vacío
- idx 1: Cant
- idx 2: Corre.
- idx 3: BENEFICIARIO (también encabezado GRUPO N)
- idx 4: RUT
- idx 7: TIPOLOGIA VIVIENDA
- idx 8: INICIO (DD/M/YYYY)
- idx 9: TERMINO
- idx 10: DIAS
"""
...
CHART_NAMES = [
    "CurvaS_GRUPO_1_Pinchulaf.png",
    "CurvaS_TOTAL_Pinchulaf.png",
    "CurvaS_Todos_Grupos_Pinchulaf.png",
]

orden_grupos = ["GRUPO 1"]

BENEFICIARIOS_SETUP = [
    ("PICHULAF LAFQUEN ELSA", "GRUPO 1", "17/08/2026"),
]

EXCLUIR_APELLIDOS = set()

# ─────────────────────────────────────────────────────────────────────────────
# LOG
# ─────────────────────────────────────────────────────────────────────────────
log = _ccu.setup_logging("pinchulaf")
...
def setup_hojas(sheets_svc):
    ...
    if not check.get("values"):
        ...
        log.info(f"'Datos Control' poblada con {len(BENEFICIARIOS_SETUP)} beneficiarias.")


# ─────────────────────────────────────────────────────────────────────────────
# 2b. SINCRONIZAR GRUPOS DESDE GANTT
# Pinchulaf: GRUPO en idx 3, nombre completo en idx 3, INICIO en idx 8
# ─────────────────────────────────────────────────────────────────────────────
...
def _parse_inicio_gantt(raw):
    """Convierte fecha DD/M/YYYY o MM/DD/YYYY a DD/MM/YYYY."""
    raw = raw.strip()
    if not raw:
        return raw
...
def sincronizar_grupos_desde_gantt(sheets_svc):
    ...
    r = sheets_svc.spreadsheets().values().get(
        spreadsheetId=SPREADSHEET_ID,
        range="'Programa de obra'!A1:BZ60",
    ...
    for row in prog_rows:
        if not row:
            continue
        # GRUPO header en idx 3
        cell_3 = str(row[3]).strip().upper() if len(row) > 3 else ""
        if cell_3.startswith("GRUPO"):
            grupo_actual = cell_3
            continue
        # Nombre completo en idx 3, INICIO en idx 8
        nombre_raw = str(row[3]).strip() if len(row) > 3 else ""
        inicio_raw = _parse_inicio_gantt(str(row[8]).strip() if len(row) > 8 else "")
...
def leer_avance_appsheet():
    if not APPSHEET_DISPONIBLE:
        return None
    try:
        log.info(f"Leyendo avance desde AppSheet {APPSHEET_PROJECT} (Pinchulaf)...")
        datos = leer_avance_proyecto(APPSHEET_PROJECT)
        result = {_normalizar_nombre(k): v for k, v in datos.items()}
        log.info(f"  AppSheet {APPSHEET_PROJECT}: {len(result)} beneficiarias leídas.")
        return result
...
def leer_datos_control(sheets_svc):
    ...
    log.info(f"Beneficiarias: {sum(len(v) for v in grupos.values())} en {len(grupos)} grupos | avance desde: {fuente}")
...
def generar_grafico_grupo(nombre_grupo, beneficiarios, control, outdir, pct_prog_gantt=None):
    ...
    ax.set_title(
        f"Curva S - {nombre_grupo} · Elsa Pinchulaf ({len(beneficiarios)} vivienda)\n"
    ...
    plt.tight_layout()
    fname = Path(outdir) / f"CurvaS_{nombre_grupo.replace(' ', '_')}_Pinchulaf.png"
...
def generar_grafico_total(grupos, control, fines_proy_global, outdir, pct_prog_gantt=None):
    ...
    ax2.set_title(
        f"Curva S Total - Elsa Pinchulaf (1 vivienda)\n"
    ...
    plt.tight_layout()
    fname_total = Path(outdir) / "CurvaS_TOTAL_Pinchulaf.png"
...
def generar_grafico_todos(grupos, control, fines_proy_global, outdir):
    ...
    ax3.set_title(
        "Curva S por Grupo - Elsa Pinchulaf\n"
    ...
    plt.tight_layout()
    fname = Path(outdir) / "CurvaS_Todos_Grupos_Pinchulaf.png"
    plt.savefig(fname, dpi=DPI_EXPORT, bbox_inches="tight"); plt.close()
    log.info(f"  Grafico guardado: {fname.name}")
...
# ─────────────────────────────────────────────────────────────────────────────
def _load_drive_ids():
    return _ccu.load_drive_ids("pinchulaf")


def _save_drive_ids(ids):
    _ccu.save_drive_ids("pinchulaf", ids)


def actualizar_drive(drive_svc, outdir):
    return _ccu.actualizar_drive_organizado(drive_svc, outdir, CHART_NAMES, OBRA_FOLDER, "pinchulaf")
...
def insertar_imagenes_en_sheets(sheets_svc, drive_ids):
    log.info("Actualizando formulas IMAGE() en hoja 'Curvas S' Pinchulaf...")
    meta = sheets_svc.spreadsheets().get(spreadsheetId=SPREADSHEET_ID).execute()
    curvas_gid = None
    for s in meta["sheets"]:
    ...
    W_PX, H_PX = 755, 385
    layout = [
        ("CurvaS_GRUPO_1_Pinchulaf.png",     0, 0),
        ("CurvaS_TOTAL_Pinchulaf.png",        1, 0),
        ("CurvaS_Todos_Grupos_Pinchulaf.png", 0, 1),
    ]
...
def main():
    ...
    log.info("=" * 65)
    log.info(f"INICIO CURVAS S ELSA PINCHULAF  -  {ts}")
    log.info("=" * 65)
    try:
        ...
        log.info("=" * 65)
        log.info("COMPLETADO. Curvas S Elsa Pinchulaf actualizadas.")
        log.info("=" * 65)
