"""
curvas_automatico_quilaleo.py  —  Curvas S automáticas · Individual Sonia Quilaleo (P116)
==========================================================================================
- Fecha de control: date.today()
- Datos de avance: AppSheet P116 (fallback: hoja 'Datos Control')
...
# CONFIGURACION
# ─────────────────────────────────────────────────────────────────────────────
SPREADSHEET_ID   = "1z9kNq9uo363NrWqCojGfGpP326FDj3V60irWxtMMbU8"
GANTT_HOJA       = "Programa de obra"
APPSHEET_PROJECT = "P116"
OBRA_FOLDER    = "Quilaleo"
...
# Curva S programada (147 dias = 21 semanas efectivas, offset semana 1)
...
CHART_NAMES = [
    "CurvaS_GRUPO_1_Quilaleo.png",
    "CurvaS_TOTAL_Quilaleo.png",
    "CurvaS_Todos_Grupos_Quilaleo.png",
]

orden_grupos = ["GRUPO 1"]

BENEFICIARIOS_SETUP = [
    ("SONIA QUILALEO TRINA", "GRUPO 1", "16/03/2026"),
]

# ─────────────────────────────────────────────────────────────────────────────
# LOG
# ─────────────────────────────────────────────────────────────────────────────
log = _ccu.setup_logging("quilaleo")
...
def leer_avance_appsheet():
    if not APPSHEET_DISPONIBLE:
        return None
    try:
        log.info(f"Leyendo avance desde AppSheet {APPSHEET_PROJECT} (Sonia Quilaleo)...")
...
def generar_grafico_grupo(nombre_grupo, beneficiarios, control, outdir, pct_prog_gantt=None):
    ...
    ax.set_title(
        f"Curva S - {nombre_grupo} · Sonia Quilaleo ({len(beneficiarios)} vivienda)\n"
    ...
    plt.tight_layout()
    fname = Path(outdir) / f"CurvaS_{nombre_grupo.replace(' ', '_')}_Quilaleo.png"
...
def generar_grafico_total(grupos, control, fines_proy_global, outdir, pct_prog_gantt=None):
    ...
    ax2.set_title(
        f"Curva S Total - Sonia Quilaleo ({len(todos)} vivienda)\n"
    ...
    plt.tight_layout()
    fname_total = Path(outdir) / "CurvaS_TOTAL_Quilaleo.png"
...
def generar_grafico_todos(grupos, control, fines_proy_global, outdir):
    ...
    ax3.set_title(
        "Curva S por Grupo - Sonia Quilaleo\n"
    ...
    plt.tight_layout()
    fname = Path(outdir) / "CurvaS_Todos_Grupos_Quilaleo.png"
    plt.savefig(fname, dpi=DPI_EXPORT, bbox_inches="tight"); plt.close()
    log.info(f"  Grafico guardado: {fname.name}")
...
# ─────────────────────────────────────────────────────────────────────────────
def _load_drive_ids():
    return _ccu.load_drive_ids("quilaleo")


def _save_drive_ids(ids):
    _ccu.save_drive_ids("quilaleo", ids)


def actualizar_drive(drive_svc, outdir):
    return _ccu.actualizar_drive_organizado(drive_svc, outdir, CHART_NAMES, OBRA_FOLDER, "quilaleo")
...
def insertar_imagenes_en_sheets(sheets_svc, drive_ids):
    log.info("Actualizando formulas IMAGE() en hoja 'Curvas S' Sonia Quilaleo...")
    meta = sheets_svc.spreadsheets().get(spreadsheetId=SPREADSHEET_ID).execute()
    curvas_gid = None
    for s in meta["sheets"]:
    ...
    W_PX, H_PX = 755, 385
    layout = [
        ("CurvaS_GRUPO_1_Quilaleo.png",     0, 0),
        ("CurvaS_TOTAL_Quilaleo.png",        1, 0),
        ("CurvaS_Todos_Grupos_Quilaleo.png", 0, 1),
    ]
...
def main():
    ...
    log.info("=" * 65)
    log.info(f"INICIO CURVAS S SONIA QUILALEO  -  {ts}")
    log.info("=" * 65)
    try:
        ...
        log.info("=" * 65)
        log.info("COMPLETADO. Curvas S Sonia Quilaleo actualizadas.")
        log.info("=" * 65)
