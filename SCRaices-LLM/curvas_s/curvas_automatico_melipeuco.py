"""
curvas_automatico_melipeuco.py  —  Curvas S automáticas · Raíces de Melipeuco (P131)
=====================================================================================
- Fecha de control: date.today()
- Datos de avance: AppSheet P131 (fallback: hoja 'Datos Control')
- 2 grupos · 14 beneficiarios
- Genera 4 PNGs: GRUPO 1 + GRUPO 2 + total + todos grupos
- Actualiza Google Drive y hoja 'Curvas S' en el Gantt de control

Estructura: GRUPO/BENEFICIARIO en idx 2 (col C), INICIO en idx 6 (col G).
"""
...
# CONFIGURACION
# ─────────────────────────────────────────────────────────────────────────────
SPREADSHEET_ID   = "1n5F-P5cy8Wj5BujllzdnwCrKwGsfIkdvHxcyd6YwscU"
GANTT_HOJA       = "Programa de obra"
APPSHEET_PROJECT = "P131"
OBRA_FOLDER    = "Melipeuco"
...
CHART_NAMES = [
    "CurvaS_GRUPO_1_Melipeuco.png",
    "CurvaS_GRUPO_2_Melipeuco.png",
    "CurvaS_TOTAL_Melipeuco.png",
    "CurvaS_Todos_Grupos_Melipeuco.png",
]

orden_grupos = ["GRUPO 1", "GRUPO 2"]
...
# LOG
# ─────────────────────────────────────────────────────────────────────────────
log = _ccu.setup_logging("melipeuco")
...
# Melipeuco: GRUPO en idx 3, BENEFICIARIO en idx 3, INICIO en idx 8
# ─────────────────────────────────────────────────────────────────────────────
...
def _reordenar_datos_control(sheets_svc):
    ...
    sheets_svc.spreadsheets().values().clear(
        spreadsheetId=SPREADSHEET_ID, range="'Datos Control'!A1:D100"
    ).execute()
    sheets_svc.spreadsheets().values().update(
        spreadsheetId=SPREADSHEET_ID,
        range="'Datos Control'!A1",
...
def sincronizar_grupos_desde_gantt(sheets_svc):
    ...
    for row in prog_rows:
        ...
        # INICIO en idx 6 (col G) tras eliminar columna vacía inicial
        inicio_raw = str(row[6]).strip() if len(row) > 6 else ""
    ...
    for i, row in enumerate(dc_rows[4:], start=5):
        ...
        if nombre_norm in gantt_map:
            ...
            fecha_dc_d   = _parse_fecha(inicio_dc_s)
            fecha_gtt_d  = _parse_fecha(inicio_gantt)
            fecha_cambio = (fecha_dc_d is not None and fecha_gtt_d is not None
                            and fecha_dc_d != fecha_gtt_d)
            if grupo_cambio or fecha_cambio:
                cambios.append({
                    ...
                    "inicio_dc":      inicio_dc_s,
                    "inicio_nuevo":   inicio_gantt,
                    "grupo_cambio":   grupo_cambio,
    ...
    for c in cambios:
        motivo = []
        if c["grupo_cambio"]:
            motivo.append(f"grupo {c['grupo_anterior']}→{c['grupo_nuevo']}")
        if c["fecha_cambio"]:
            motivo.append(f"fecha {c['inicio_dc']}→{c['inicio_nuevo']}")
        log.info(f"    >> {c['nombre']}: {', '.join(motivo)}")
...
def leer_avance_appsheet():
    if not APPSHEET_DISPONIBLE:
        return None
    try:
        log.info(f"Leyendo avance desde AppSheet {APPSHEET_PROJECT} (Raíces de Melipeuco)...")
...
def generar_grafico_grupo(nombre_grupo, beneficiarios, control, outdir, pct_prog_gantt=None):
    ...
    ax.set_title(
        f"Curva S - {nombre_grupo} · Raíces de Melipeuco ({len(beneficiarios)} viviendas)\n"
    ...
    plt.tight_layout()
    safe = nombre_grupo.replace(" ", "_")
    fname = Path(outdir) / f"CurvaS_{safe}_Melipeuco.png"
...
def generar_grafico_total(grupos, control, fines_proy_global, outdir, pct_prog_gantt=None):
    todos = [b for g in orden_grupos for b in grupos.get(g, [])]
    ...
    ax2.set_title(
        f"Curva S Total - Raíces de Melipeuco ({len(todos)} viviendas)\n"
    ...
    plt.tight_layout()
    fname_total = Path(outdir) / "CurvaS_TOTAL_Melipeuco.png"
...
def generar_grafico_todos(grupos, control, fines_proy_global, outdir):
    ...
    for nombre_grupo in orden_grupos:
        beneficiarios = grupos.get(nombre_grupo, [])
    ...
    ax3.set_title(
        "Curva S por Grupo - Raíces de Melipeuco\n"
    ...
    plt.tight_layout()
    fname = Path(outdir) / "CurvaS_Todos_Grupos_Melipeuco.png"
    plt.savefig(fname, dpi=DPI_EXPORT, bbox_inches="tight"); plt.close()
    log.info(f"  Grafico guardado: {fname.name}")
...
# ─────────────────────────────────────────────────────────────────────────────
def _load_drive_ids():
    return _ccu.load_drive_ids("melipeuco")


def _save_drive_ids(ids):
    _ccu.save_drive_ids("melipeuco", ids)


def actualizar_drive(drive_svc, outdir):
    return _ccu.actualizar_drive_organizado(drive_svc, outdir, CHART_NAMES, OBRA_FOLDER, "melipeuco")
...
def insertar_imagenes_en_sheets(sheets_svc, drive_ids):
    log.info("Actualizando formulas IMAGE() en hoja 'Curvas S' Raíces de Melipeuco...")
    meta = sheets_svc.spreadsheets().get(spreadsheetId=SPREADSHEET_ID).execute()
    curvas_gid = None
    for s in meta["sheets"]:
    ...
    W_PX, H_PX = 755, 385
    layout = [
        ("CurvaS_TOTAL_Melipeuco.png",         0, 0),
        ("CurvaS_Todos_Grupos_Melipeuco.png",   1, 0),
        ("CurvaS_GRUPO_1_Melipeuco.png",        0, 1),
        ("CurvaS_GRUPO_2_Melipeuco.png",        1, 1),
    ]
...
def main():
    ...
    log.info("=" * 65)
    log.info(f"INICIO CURVAS S RAICES DE MELIPEUCO  -  {ts}")
    log.info("=" * 65)
    try:
        ...
        for nombre_grupo in orden_grupos:
            if nombre_grupo not in grupos:
                continue
            _, _, _, fin_proy = generar_grafico_grupo(nombre_grupo, grupos[nombre_grupo], control_date, OUTPUT_DIR, pct_prog_gantt=pct_prog_por_grupo.get(nombre_grupo.upper()))
        ...
        log.info("=" * 65)
        log.info("COMPLETADO. Curvas S Raíces de Melipeuco actualizadas.")
        log.info("=" * 65)
