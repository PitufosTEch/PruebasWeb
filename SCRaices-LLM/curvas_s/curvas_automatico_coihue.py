"""
curvas_automatico_coihue.py  —  Curvas S automáticas · El Coihue (P39)
=====================================================================
- Fecha de control: date.today()
- Datos de avance: AppSheet P39 (fallback: hoja 'Datos Control')
- 3 grupos · 21 beneficiarios
- Genera 5 PNGs: grupo 1-3 + total + todos grupos
...
# Curva S programada individual (56 semanas = 392 días)
PCT_SEMANA = [
...
CHART_NAMES = [
    "CurvaS_GRUPO_1_Coihue.png",
    "CurvaS_GRUPO_2_Coihue.png",
    "CurvaS_GRUPO_3_Coihue.png",
    "CurvaS_TOTAL_Coihue.png",
    "CurvaS_Todos_Grupos_Coihue.png",
]
...
# ─────────────────────────────────────────────────────────────────────────────
def _get_or_create_sheet(sheets_svc, title):
...
def setup_hojas(sheets_svc):
    ...
    # Poblar Datos Control si está vacía
...
def _reordenar_datos_control(sheets_svc):
    ...
    orden_grupos = ["GRUPO 1", "GRUPO 2", "GRUPO 3"]
...
def leer_avance_appsheet():
    if not APPSHEET_DISPONIBLE:
        return None
    try:
        log.info(f"Leyendo avance desde AppSheet {APPSHEET_PROJECT} (El Coihue)...")
...
def generar_grafico_grupo(nombre_grupo, beneficiarios, control, outdir):
    ...
    ax.set_title(
        f"Curva S - {nombre_grupo} · El Coihue ({len(beneficiarios)} viviendas)\n"
    ...
    plt.tight_layout()
    fname = Path(outdir) / f"CurvaS_{nombre_grupo.replace(' ', '_')}_Coihue.png"
...
def generar_grafico_total(grupos, control, fines_proy_global, outdir):
    ...
    ax2.set_title(
        f"Curva S Total - El Coihue ({len(todos)} viviendas)\n"
    ...
    plt.tight_layout()
    fname_total = Path(outdir) / "CurvaS_TOTAL_Coihue.png"
...
def generar_grafico_todos(grupos, control, fines_proy_global, outdir):
    ...
    plt.tight_layout()
    fname = Path(outdir) / "CurvaS_Todos_Grupos_Coihue.png"
    plt.savefig(fname, dpi=DPI_EXPORT, bbox_inches="tight"); plt.close()
    log.info(f"  Gráfico guardado: {fname.name}")
...
# ─────────────────────────────────────────────────────────────────────────────
def _load_drive_ids():
    return _ccu.load_drive_ids("coihue")


def _save_drive_ids(ids):
    _ccu.save_drive_ids("coihue", ids)
...
def insertar_imagenes_en_sheets(sheets_svc, drive_ids):
    log.info("Actualizando fórmulas IMAGE() en hoja 'Curvas S' El Coihue...")
    meta = sheets_svc.spreadsheets().get(spreadsheetId=SPREADSHEET_ID).execute()
    curvas_gid = None
    for s in meta["sheets"]:
    ...
    W_PX, H_PX = 755, 385
    # Layout 2 columnas: grupos en filas 0-1, total+todos en fila 2
    layout = [
        ("CurvaS_GRUPO_1_Coihue.png",      0, 0),
        ("CurvaS_GRUPO_2_Coihue.png",      1, 0),
        ("CurvaS_GRUPO_3_Coihue.png",      0, 1),
        ("CurvaS_TOTAL_Coihue.png",        1, 1),
        ("CurvaS_Todos_Grupos_Coihue.png", 0, 2),
    ]
...
def main():
    ...
    log.info("=" * 65)
    log.info(f"INICIO CURVAS S EL COIHUE  -  {ts}")
    log.info("=" * 65)
    try:
        ...
        log.info("=" * 65)
        log.info("COMPLETADO. Curvas S El Coihue actualizadas.")
        log.info("=" * 65)
