"""
curvas_automatico_trovolhue.py  —  Curvas S automáticas · Trovolhue (P31)
=========================================================================
- Fecha de control: date.today()
- Datos de avance: AppSheet P31 (fallback: hoja 'Datos Control')
- 1 grupo · 2 beneficiarios
- Genera 3 PNGs: GRUPO 1 + total + todos grupos
- Actualiza Google Drive y hoja 'Curvas S' en el Gantt de control

Diferencias estructurales vs otros proyectos:
- GRUPO en idx 2 (col C), APELLIDOS idx 2 + NOMBRES idx 3
- INICIO en idx 4 (col E), fechas en formato MM/DD/YYYY en la hoja
"""
...
# CONFIGURACION
# ─────────────────────────────────────────────────────────────────────────────
SPREADSHEET_ID   = "1kgroktRIto3gGGnvmXGMgoetXT-Rv8JFue1K9eNlNMg"
GANTT_HOJA       = "Programa de obra"
APPSHEET_PROJECT = "P31"
OBRA_FOLDER    = "Trovolhue"
...
# Curva S programada (fila 9 del Gantt, proyecto promedio)
...
CHART_NAMES = [
    "CurvaS_GRUPO_1_Trovolhue.png",
    "CurvaS_TOTAL_Trovolhue.png",
    "CurvaS_Todos_Grupos_Trovolhue.png",
]

orden_grupos = ["GRUPO 1"]

BENEFICIARIOS_SETUP = [
    ("CASTRO CAYUPAN MARIA VICTORIA", "GRUPO 1", "03/11/2025"),
    ("TRALMA HUILIPAN IVAN ADIEL",    "GRUPO 1", "17/11/2025"),
]

# Beneficiarios a ignorar completamente (AppSheet + Gantt)
EXCLUIR_APELLIDOS = {"FIERRO"}

# ─────────────────────────────────────────────────────────────────────────────
# LOG
# ─────────────────────────────────────────────────────────────────────────────
log = _ccu.setup_logging("trovolhue")
...
# ─────────────────────────────────────────────────────────────────────────────
# 2. SETUP HOJAS (primera ejecucion)
# ─────────────────────────────────────────────────────────────────────────────
...
# Trovolhue: GRUPO en idx 2, APELLIDOS+NOMBRES en idx 2+3, INICIO en idx 4
# ─────────────────────────────────────────────────────────────────────────────
...
def _parse_inicio_gantt(raw):
    """Convierte fecha MM/DD/YYYY o DD/MM/YYYY a DD/MM/YYYY."""
    raw = raw.strip()
    if not raw:
        return raw
    # Intentar MM/DD/YYYY primero (formato de la hoja Trovolhue)
...
def sincronizar_grupos_desde_gantt(sheets_svc):
    ...
    r = sheets_svc.spreadsheets().values().get(
        spreadsheetId=SPREADSHEET_ID,
        range="'Programa de obra'!A1:BG60",
    ...
    for row in prog_rows:
        if not row:
            continue
        # Trovolhue: GRUPO header en idx 2
        for col_idx in [2, 3, 0]:
            if col_idx < len(row):
                cell = str(row[col_idx]).strip().upper()
                if cell.startswith("GRUPO"):
                    grupo_actual = cell
                    break
        # Nombre = APELLIDOS (idx 2) + NOMBRES (idx 3)
        apellidos_raw = str(row[2]).strip() if len(row) > 2 else ""
        nombres_raw   = str(row[3]).strip() if len(row) > 3 else ""
        nombre_raw    = (apellidos_raw + " " + nombres_raw).strip() if nombres_raw else apellidos_raw
        # INICIO en idx 4 (col E)
        inicio_raw_orig = str(row[4]).strip() if len(row) > 4 else ""
        inicio_raw = _parse_inicio_gantt(inicio_raw_orig)

        if (not nombre_raw or nombre_raw.upper().startswith("GRUPO")
                or nombre_raw.upper() in ("BENEFICIARIO", "CARTA GANTT", "BENEFICIARIO ", "")
                or not grupo_actual):
            continue
        try:
        ...
        primer_token = nombre_norm.split()[0] if nombre_norm else ""
        if nombre_norm and primer_token not in EXCLUIR_APELLIDOS:
...
def leer_avance_appsheet():
    if not APPSHEET_DISPONIBLE:
        return None
    try:
        log.info(f"Leyendo avance desde AppSheet {APPSHEET_PROJECT} (Trovolhue)...")
...
def leer_datos_control(sheets_svc):
    ...
    for row in rows[4:]:
        ...
        if not grupo_raw or not nombre:
            continue
        # Excluir beneficiarios descartados explícitamente
        primer_apellido = _normalizar_nombre(nombre).split()[0] if nombre else ""
        if primer_apellido in EXCLUIR_APELLIDOS:
            log.info(f"  Excluido del cálculo: {nombre}")
            continue
...
def generar_grafico_grupo(nombre_grupo, beneficiarios, control, outdir, pct_prog_gantt=None):
    ...
    ax.set_title(
        f"Curva S - {nombre_grupo} · Trovolhue ({len(beneficiarios)} viviendas)\n"
    ...
    plt.tight_layout()
    fname = Path(outdir) / f"CurvaS_{nombre_grupo.replace(' ', '_')}_Trovolhue.png"
...
def generar_grafico_total(grupos, control, fines_proy_global, outdir, pct_prog_gantt=None):
    ...
    ax2.axvline(fin_proy_total, color="#9467bd", linewidth=2, linestyle=":", label=f"Termino Proyectado ({_fmt_date(fin_proy_total)})", zorder=4)
    ax2.axvline(fin_prog_total, color="#1f77b4", linewidth=1.5, linestyle=":", alpha=0.6, label=f"Termino Programado ({_fmt_date(fin_prog_total)})", zorder=3)
    ...
    ax2.set_title(
        f"Curva S Total - Trovolhue ({len(todos)} viviendas)\n"
    ...
    plt.tight_layout()
    fname_total = Path(outdir) / "CurvaS_TOTAL_Trovolhue.png"
...
def generar_grafico_todos(grupos, control, fines_proy_global, outdir):
    ...
    plt.tight_layout()
    fname = Path(outdir) / "CurvaS_Todos_Grupos_Trovolhue.png"
    plt.savefig(fname, dpi=DPI_EXPORT, bbox_inches="tight"); plt.close()
    log.info(f"  Grafico guardado: {fname.name}")
...
# ─────────────────────────────────────────────────────────────────────────────
def _load_drive_ids():
    return _ccu.load_drive_ids("trovolhue")


def _save_drive_ids(ids):
    _ccu.save_drive_ids("trovolhue", ids)


def actualizar_drive(drive_svc, outdir):
    return _ccu.actualizar_drive_organizado(drive_svc, outdir, CHART_NAMES, OBRA_FOLDER, "trovolhue")
...
def insertar_imagenes_en_sheets(sheets_svc, drive_ids):
    log.info("Actualizando formulas IMAGE() en hoja 'Curvas S' Trovolhue...")
    meta = sheets_svc.spreadsheets().get(spreadsheetId=SPREADSHEET_ID).execute()
    curvas_gid = None
    for s in meta["sheets"]:
    ...
    W_PX, H_PX = 755, 385
    layout = [
        ("CurvaS_GRUPO_1_Trovolhue.png",     0, 0),
        ("CurvaS_TOTAL_Trovolhue.png",        1, 0),
        ("CurvaS_Todos_Grupos_Trovolhue.png", 0, 1),
    ]
...
def main():
    ...
    log.info("=" * 65)
    log.info(f"INICIO CURVAS S TROVOLHUE  -  {ts}")
    log.info("=" * 65)
    try:
        ...
        log.info("=" * 65)
        log.info("COMPLETADO. Curvas S Trovolhue actualizadas.")
        log.info("=" * 65)
