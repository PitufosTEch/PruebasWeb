"""
curvas_automatico_cunco.py  —  Curvas S automáticas · Nuevo Cunco (P127)
=========================================================================
- Fecha de control: date.today()
- Datos de avance: AppSheet P127 (fallback: hoja 'Datos Control')
- 4 grupos · 25 beneficiarios
- Genera 6 PNGs: grupo 1-4 + total + todos grupos
...
# CONFIGURACION
# ─────────────────────────────────────────────────────────────────────────────
SPREADSHEET_ID   = "1bgT-83Aea0DlyeQ6OvitGDZfm3jLRQUV0GooVP-G8EI"
GANTT_HOJA       = "Programa de obra"
APPSHEET_PROJECT = "P127"
OBRA_FOLDER    = "Nuevo Cunco"
...
CHART_NAMES = [
    "CurvaS_GRUPO_1_Cunco.png",
    "CurvaS_GRUPO_2_Cunco.png",
    "CurvaS_GRUPO_3_Cunco.png",
    "CurvaS_GRUPO_4_Cunco.png",
    "CurvaS_TOTAL_Cunco.png",
    "CurvaS_Todos_Grupos_Cunco.png",
]

# Beneficiarios con fechas de inicio individuales según Programa de obra
BENEFICIARIOS_SETUP = [
    # GRUPO 1 — inicios escalonados desde 22/06/2026
    ("VILLAGRA MUÑOZ NORMA",                   "GRUPO 1", "22/06/2026"),
    ("SANDOVAL CURRIHUIL NORMA ISABEL",        "GRUPO 1", "22/06/2026"),
    ("RAÑIQUEO HUENCHULAF MARINA DEL CARMEN",  "GRUPO 1", "29/06/2026"),
    ("ORMEÑO ARIAS MELANIA DEL CARMEN",        "GRUPO 1", "29/06/2026"),
    ("CÓRDOVA EYZAGUIRRE ZULEMA",              "GRUPO 1", "06/07/2026"),
    ("MELGAREJO LEIVA CARMEN DE DIOS",         "GRUPO 1", "06/07/2026"),
    ("NAHUELVIL GALLARDO MARINA ROSA",         "GRUPO 1", "13/07/2026"),
    # GRUPO 2 — inicios escalonados desde 20/07/2026
    ("CAYUMÁN PEREIRA SOFIA ISABEL",           "GRUPO 2", "20/07/2026"),
    ("HUENCHULAF CANIUMIL HONORINDO",          "GRUPO 2", "20/07/2026"),
    ("CATRILAF MORALES PABLO RENATO",          "GRUPO 2", "27/07/2026"),
    ("CATRILAF LEVIAN MANUEL ARAUCO",          "GRUPO 2", "27/07/2026"),
    ("MORALES PERALTA ELISA",                  "GRUPO 2", "03/08/2026"),
    ("SCHAFER HELMUT KURT GEORG",              "GRUPO 2", "03/08/2026"),
    ("DÍAZ ARAVENA YASNA VICTORIANA",          "GRUPO 2", "10/08/2026"),
    ("CONCHA REYES JOSÉ GERÓNIMO",             "GRUPO 2", "10/08/2026"),
    # GRUPO 3 — inicios escalonados desde 17/08/2026
    ("MORALES ANCAMIL ELEUTERIO",              "GRUPO 3", "17/08/2026"),
    ("LINCOLEO HUILCALEO ALBERTINA DEL CARMEN","GRUPO 3", "17/08/2026"),
    ("MORALES CURIPÁN EVA FLORINDA",           "GRUPO 3", "24/08/2026"),
    ("VILUGRÓN RÍOS ANA LUISA",                "GRUPO 3", "24/08/2026"),
    ("YAÑEZ VEGA REINALDO HUMBERTO",           "GRUPO 3", "31/08/2026"),
    ("MOLINA MOLINA HENRY GUIDO",              "GRUPO 3", "31/08/2026"),
    # GRUPO 4 — inicios escalonados desde 07/09/2026
    ("GUÍÑEZ CEA EMILIO ARTURO",               "GRUPO 4", "07/09/2026"),
    ("CERDA PEREDA ANGEL CUSTODIO",            "GRUPO 4", "07/09/2026"),
    ("MAUREIRA OJEDA ISAÍAS DEL CARMEN",       "GRUPO 4", "14/09/2026"),
    ("RAMÍREZ ACUÑA ISABEL MAGDALENA",         "GRUPO 4", "14/09/2026"),
]

# ─────────────────────────────────────────────────────────────────────────────
# LOG
# ─────────────────────────────────────────────────────────────────────────────
log = _ccu.setup_logging("cunco")
...
def _reordenar_datos_control(sheets_svc):
    ...
    orden_grupos = ["GRUPO 1", "GRUPO 2", "GRUPO 3", "GRUPO 4"]
...
def leer_avance_appsheet():
    if not APPSHEET_DISPONIBLE:
        return None
    try:
        log.info(f"Leyendo avance desde AppSheet {APPSHEET_PROJECT} (Nuevo Cunco)...")
...
def generar_grafico_grupo(nombre_grupo, beneficiarios, control, outdir, pct_prog_gantt=None):
    ...
    ax.set_title(
        f"Curva S - {nombre_grupo} · Nuevo Cunco ({len(beneficiarios)} viviendas)\n"
    ...
    plt.tight_layout()
    fname = Path(outdir) / f"CurvaS_{nombre_grupo.replace(' ', '_')}_Cunco.png"
...
def generar_grafico_total(grupos, control, fines_proy_global, outdir, pct_prog_gantt=None):
    ...
    ax2.set_title(
        f"Curva S Total - Nuevo Cunco ({len(todos)} viviendas)\n"
    ...
    plt.tight_layout()
    fname_total = Path(outdir) / "CurvaS_TOTAL_Cunco.png"
...
def generar_grafico_todos(grupos, control, fines_proy_global, outdir):
    ...
    plt.tight_layout()
    fname = Path(outdir) / "CurvaS_Todos_Grupos_Cunco.png"
    plt.savefig(fname, dpi=DPI_EXPORT, bbox_inches="tight"); plt.close()
    log.info(f"  Gráfico guardado: {fname.name}")
...
# ─────────────────────────────────────────────────────────────────────────────
def _load_drive_ids():
    return _ccu.load_drive_ids("cunco")


def _save_drive_ids(ids):
    _ccu.save_drive_ids("cunco", ids)


def actualizar_drive(drive_svc, outdir):
    return _ccu.actualizar_drive_organizado(drive_svc, outdir, CHART_NAMES, OBRA_FOLDER, "cunco")
...
def insertar_imagenes_en_sheets(sheets_svc, drive_ids):
    log.info("Actualizando fórmulas IMAGE() en hoja 'Curvas S' Nuevo Cunco...")
    meta = sheets_svc.spreadsheets().get(spreadsheetId=SPREADSHEET_ID).execute()
    curvas_gid = None
    for s in meta["sheets"]:
    ...
    W_PX, H_PX = 755, 385
    # Layout 2 columnas: grupos 1-4 en filas 0-1, total+todos en fila 2
    layout = [
        ("CurvaS_GRUPO_1_Cunco.png",      0, 0),
        ("CurvaS_GRUPO_2_Cunco.png",      1, 0),
        ("CurvaS_GRUPO_3_Cunco.png",      0, 1),
        ("CurvaS_GRUPO_4_Cunco.png",      1, 1),
        ("CurvaS_TOTAL_Cunco.png",        0, 2),
        ("CurvaS_Todos_Grupos_Cunco.png", 1, 2),
    ]
    ...
    log.info(f"  Fórmulas IMAGE() actualizadas (v={ts}).")
...
def main():
    ...
    log.info("=" * 65)
    log.info(f"INICIO CURVAS S NUEVO CUNCO  -  {ts}")
    log.info("=" * 65)
    try:
        ...
        log.info("=" * 65)
        log.info("COMPLETADO. Curvas S Nuevo Cunco actualizadas.")
        log.info("=" * 65)
