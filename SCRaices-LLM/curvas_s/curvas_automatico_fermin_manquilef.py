"""
curvas_automatico_fermin_manquilef.py  -  Automatizacion semanal Curvas S - Proyecto P132 Com. Fermin Manquilef
===============================================================================================================
- Fecha de control:  date.today() (automatica)
- Datos de avance:   leidos desde AppSheet (proyecto P132)
...
Ejecucion manual:  python curvas_automatico_fermin_manquilef.py
"""
...
# CONFIGURACION
# ─────────────────────────────────────────────────────────────────────────────
SPREADSHEET_ID   = "1RTWSEQRggmcQCepGnXohJrYtH1xC_WgTrfB88Bip5KY"
APPSHEET_PROJECT = "P132"
TOKEN_FILE       = _ccu.TOKEN_FILE
OBRA_FOLDER      = "Fermin Manquilef"
...
CHART_NAMES = [
    "CurvaS_GRUPO_1_Manquilef.png",
    "CurvaS_GRUPO_2_Manquilef.png",
    "CurvaS_GRUPO_3_Manquilef.png",
    "CurvaS_TOTAL_Manquilef.png",
    "CurvaS_Todos_Grupos_Manquilef.png",
]

# Datos hardcodeados desde Gantt; usados como fallback cuando Datos Control no existe
BENEFICIARIOS_SETUP = [
    # GRUPO 1
    ("GRUPO 1", "Ámbar Patricia Álvarez Lemuguir",         "14/12/2026"),
    ("GRUPO 1", "Verónica Isabel Epuñán Huinca",           "14/12/2026"),
    ("GRUPO 1", "Patricia Mireya Huilcán Lemuguir",        "21/12/2026"),
    # GRUPO 2
    ("GRUPO 2", "Carola Del Pilar Huinca Huinca",          "21/12/2026"),
    ("GRUPO 2", "Jocelyn Marcela Carvajal Huinca",         "28/12/2026"),
    ("GRUPO 2", "Eliana Luz Huinca Flores",                "28/12/2026"),
    ("GRUPO 2", "Julio Hugo Huinca Flores",                "04/01/2027"),
    ("GRUPO 2", "Idanea Ester Huinca Castro",              "04/01/2027"),
    ("GRUPO 2", "Magaly Mariela Huinca Marivil",           "11/01/2027"),
    ("GRUPO 2", "José Abelino Lemuñir Flores",             "11/01/2027"),
    # GRUPO 3
    ("GRUPO 3", "Margot Ivonne Manquilef Navarrete",       "18/01/2027"),
    ("GRUPO 3", "José Miguel Manquilef Navarrete",         "18/01/2027"),
    ("GRUPO 3", "Patricio Antonio Manquilef Navarrete",    "25/01/2027"),
    ("GRUPO 3", "Valeska Lucila Huinca Veloso",            "25/01/2027"),
    ("GRUPO 3", "Diego Antonio Manquilef Labrin",          "01/02/2027"),
]

log = _ccu.setup_logging("fermin_manquilef")
...
def _load_drive_ids():
    return _ccu.load_drive_ids("fermin_manquilef", {})

def _save_drive_ids(ids):
    _ccu.save_drive_ids("fermin_manquilef", ids)
...
def sincronizar_grupos_desde_gantt(sheets_svc):
    log.info("Verificando movimientos de grupo en 'Programa de obra' (Fermin Manquilef)...")
...
def leer_avance_appsheet():
    if not APPSHEET_DISPONIBLE:
        ...
        return None
    try:
        log.info(f"Leyendo avance desde AppSheet {APPSHEET_PROJECT} (Fermin Manquilef)...")
...
def generar_grafico_grupo(nombre_grupo, beneficiarios, control, outdir, pct_prog_gantt=None):
    ...
    ax.set_title(
        f"Curva S - {nombre_grupo} - Com. Fermín Manquilef P132 ({len(beneficiarios)} viviendas)\n"
    ...
    fname = Path(outdir) / f"CurvaS_{slug}_Manquilef.png"
...
def generar_grafico_total(grupos, control, fines_proy_global, outdir, pct_prog_gantt_total=None):
    ...
    ax2.set_title(
        f"Curva S Total - Com. Fermín Manquilef P132 ({len(todos)} viviendas)\n"
    ...
    plt.tight_layout()
    fname_t = Path(outdir) / "CurvaS_TOTAL_Manquilef.png"
...
def generar_grafico_todos(grupos, control, fines_proy_global, outdir):
    ...
    ax3.set_title(
        "Curva S por Grupo - Com. Fermín Manquilef P132\n"
    ...
    plt.tight_layout()
    fname = Path(outdir) / "CurvaS_Todos_Grupos_Manquilef.png"
    plt.savefig(fname, dpi=DPI_EXPORT, bbox_inches="tight"); plt.close()
    log.info(f"  Grafico guardado: {fname.name}")
...
def actualizar_drive(drive_svc, outdir, drive_ids=None):
    return _ccu.actualizar_drive_organizado(drive_svc, outdir, CHART_NAMES, OBRA_FOLDER, "fermin_manquilef")
...
def insertar_imagenes_en_sheets(sheets_svc, drive_ids):
    log.info(f"Actualizando formulas IMAGE() en hoja '{CURVA_S_SHEET}' Fermin Manquilef...")
    meta = sheets_svc.spreadsheets().get(spreadsheetId=SPREADSHEET_ID).execute()
    curva_gid = next(
    ...
    W_PX, H_PX = 755, 385
    layout = [
        ("CurvaS_GRUPO_1_Manquilef.png",     0, 0),
        ("CurvaS_GRUPO_2_Manquilef.png",     1, 0),
        ("CurvaS_GRUPO_3_Manquilef.png",     0, 1),
        ("CurvaS_TOTAL_Manquilef.png",       1, 1),
        ("CurvaS_Todos_Grupos_Manquilef.png", 0, 2),
    ]
...
def main():
    ...
    log.info("=" * 65)
    log.info(f"INICIO CURVAS S COM. FERMIN MANQUILEF P132  -  {ts}")
    log.info("=" * 65)
    ...
    try:
        ...
        log.info("Generando graficos Com. Fermin Manquilef P132...")
        fines_proy_global = []
        for nombre_grupo, beneficiarios in grupos.items():
