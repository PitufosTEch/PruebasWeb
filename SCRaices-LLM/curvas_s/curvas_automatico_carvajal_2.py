"""
curvas_automatico_carvajal_2.py  -  Automatizacion semanal Curvas S - Proyecto P154 Com. José Carvajal 2
=========================================================================================================
- Fecha de control:  date.today() (automatica)
- Datos de avance:   leidos desde AppSheet (proyecto P154)
  Fallback:          hoja 'Datos control' del spreadsheet Gantt Control Com. José Carvajal 2
- Genera 5 graficos PNG (3 grupos + total + todos grupos)
- Actualiza Drive, marca PENDIENTE para Apps Script

Ejecucion manual:  python curvas_automatico_carvajal_2.py
"""
...
# CONFIGURACION
# ─────────────────────────────────────────────────────────────────────────────
SPREADSHEET_ID   = "1nK8xaHlASKxmoGD0RPqpZdSMPMPL-bLM2MvnhBAqGHE"
APPSHEET_PROJECT = "P154"
TOKEN_FILE       = _ccu.TOKEN_FILE
OBRA_FOLDER      = "Carvajal 2"
...
CHART_NAMES = [
    "CurvaS_GRUPO_1_Carvajal2.png",
    "CurvaS_GRUPO_2_Carvajal2.png",
    "CurvaS_GRUPO_3_Carvajal2.png",
    "CurvaS_TOTAL_Carvajal2.png",
    "CurvaS_Todos_Grupos_Carvajal2.png",
]

BENEFICIARIOS_SETUP = [
    # GRUPO 1
    ("GRUPO 1", "José Bernardo Campos Carvajal",           "16/11/2026"),
    # GRUPO 2
    ("GRUPO 2", "Ariel Elías Carvajal Antilaf",            "16/11/2026"),
    ("GRUPO 2", "Esteban Enot Carvajal Ramos",             "23/11/2026"),
    ("GRUPO 2", "Evelyn Melany Carvajal Antilaf",          "23/11/2026"),
    ("GRUPO 2", "Jocelin Edith Luzmira Carvajal Antilaf",  "30/11/2026"),
    ("GRUPO 2", "Miriam Ruth Carvajal Reyes",              "30/11/2026"),
    ("GRUPO 2", "Priscila Ivett Carvajal Ramos",           "07/12/2026"),
    # GRUPO 3
    ("GRUPO 3", "Eduardo Andrés Carvajal Mellao",           "07/12/2026"),
    ("GRUPO 3", "Héctor Javier Carvajal Mellao",           "14/12/2026"),
    ("GRUPO 3", "Omar Leonardo Carvajal Mellao",           "14/12/2026"),
]

log = _ccu.setup_logging("carvajal_2")
...
def _load_drive_ids():
    return _ccu.load_drive_ids("carvajal_2", {})

def _save_drive_ids(ids):
    _ccu.save_drive_ids("carvajal_2", ids)
...
def sincronizar_grupos_desde_gantt(sheets_svc):
    log.info("Verificando movimientos de grupo en 'Programa de obra' (Carvajal 2)...")
    r = sheets_svc.spreadsheets().values().get(
        spreadsheetId=SPREADSHEET_ID,
        range="'Programa de obra'!A1:L25",
...
def leer_avance_appsheet():
    if not APPSHEET_DISPONIBLE:
        ...
        return None
    try:
        log.info(f"Leyendo avance desde AppSheet {APPSHEET_PROJECT} (Carvajal 2)...")
...
def generar_grafico_grupo(nombre_grupo, beneficiarios, control, outdir, pct_prog_gantt=None):
    ...
    ax.set_title(
        f"Curva S - {nombre_grupo} - Com. José Carvajal 2 P154 ({len(beneficiarios)} viviendas)\n"
    ...
    fname = Path(outdir) / f"CurvaS_{slug}_Carvajal2.png"
...
def generar_grafico_total(grupos, control, fines_proy_global, outdir, pct_prog_gantt_total=None):
    ...
    ax2.set_title(
        f"Curva S Total - Com. José Carvajal 2 P154 ({len(todos)} viviendas)\n"
    ...
    plt.tight_layout()
    fname_t = Path(outdir) / "CurvaS_TOTAL_Carvajal2.png"
...
def generar_grafico_todos(grupos, control, fines_proy_global, outdir):
    ...
    ax3.set_title(
        "Curva S por Grupo - Com. José Carvajal 2 P154\n"
    ...
    plt.tight_layout()
    fname = Path(outdir) / "CurvaS_Todos_Grupos_Carvajal2.png"
    plt.savefig(fname, dpi=DPI_EXPORT, bbox_inches="tight"); plt.close()
    log.info(f"  Grafico guardado: {fname.name}")
...
def actualizar_drive(drive_svc, outdir, drive_ids=None):
    return _ccu.actualizar_drive_organizado(drive_svc, outdir, CHART_NAMES, OBRA_FOLDER, "carvajal_2")
...
def insertar_imagenes_en_sheets(sheets_svc, drive_ids):
    log.info("Actualizando formulas IMAGE() en hoja 'Curva S' Carvajal 2...")
    meta     = sheets_svc.spreadsheets().get(spreadsheetId=SPREADSHEET_ID).execute()
    curva_gid = next(
    ...
    W_PX, H_PX = 755, 385
    layout = [
        ("CurvaS_GRUPO_1_Carvajal2.png",     0, 0),
        ("CurvaS_GRUPO_2_Carvajal2.png",      1, 0),
        ("CurvaS_GRUPO_3_Carvajal2.png",      0, 1),
        ("CurvaS_TOTAL_Carvajal2.png",        1, 1),
        ("CurvaS_Todos_Grupos_Carvajal2.png", 0, 2),
    ]
...
def main():
    ...
    log.info("=" * 65)
    log.info(f"INICIO CURVAS S COM. JOSE CARVAJAL 2 P154  -  {ts}")
    log.info("=" * 65)
    ...
    try:
        ...
        log.info("Generando graficos Com. José Carvajal 2 P154...")
        fines_proy_global = []
        for nombre_grupo, beneficiarios in grupos.items():
