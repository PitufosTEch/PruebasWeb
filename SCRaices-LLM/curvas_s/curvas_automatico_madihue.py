"""
curvas_automatico_madihue.py  —  Curvas S automáticas · Com. Madihue (P14)
==========================================================================
- Fecha de control: date.today()
- Datos de avance: AppSheet P14 (fallback: hoja 'Datos Control')
...
# CONFIGURACION
# ─────────────────────────────────────────────────────────────────────────────
SPREADSHEET_ID   = "1B4wO-UkIDDyFvwRYjMAGksJvKqtNblwl_IirLl3qA6E"
GANTT_HOJA       = "Programa de obra"
APPSHEET_PROJECT = "P14"
OBRA_FOLDER    = "Madihue"
...
CHART_NAMES = [
    "CurvaS_GRUPO_1_Madihue.png",
    "CurvaS_TOTAL_Madihue.png",
    "CurvaS_Todos_Grupos_Madihue.png",
]

orden_grupos = ["GRUPO 1"]

BENEFICIARIOS_SETUP = [
    ("EVELYN MARLEN CONOMAN PINTO",  "GRUPO 1", "09/03/2026"),
    ("JUAN GONZALO CONOMAN PINTO",   "GRUPO 1", "09/03/2026"),
]

# ─────────────────────────────────────────────────────────────────────────────
# LOG
# ─────────────────────────────────────────────────────────────────────────────
log = _ccu.setup_logging("madihue")
...
def sincronizar_grupos_desde_gantt(sheets_svc):
    ...
    for row in prog_rows:
        ...
        # INICIO en idx 7 (col H) — especifico de Com. Madihue
...
def leer_avance_appsheet():
    if not APPSHEET_DISPONIBLE:
        return None
    try:
        log.info(f"Leyendo avance desde AppSheet {APPSHEET_PROJECT} (Com. Madihue)...")
...
def generar_grafico_grupo(nombre_grupo, beneficiarios, control, outdir, pct_prog_gantt=None):
    ...
    ax.set_title(
        f"Curva S - {nombre_grupo} · Com. Madihue ({len(beneficiarios)} viviendas)\n"
    ...
    plt.tight_layout()
    fname = Path(outdir) / f"CurvaS_{nombre_grupo.replace(' ', '_')}_Madihue.png"
...
def generar_grafico_total(grupos, control, fines_proy_global, outdir, pct_prog_gantt=None):
    ...
    ax2.set_title(
        f"Curva S Total - Com. Madihue ({len(todos)} viviendas)\n"
    ...
    plt.tight_layout()
    fname_total = Path(outdir) / "CurvaS_TOTAL_Madihue.png"
...
def generar_grafico_todos(grupos, control, fines_proy_global, outdir):
    ...
    plt.tight_layout()
    fname = Path(outdir) / "CurvaS_Todos_Grupos_Madihue.png"
    plt.savefig(fname, dpi=DPI_EXPORT, bbox_inches="tight"); plt.close()
    log.info(f"  Grafico guardado: {fname.name}")
...
# ─────────────────────────────────────────────────────────────────────────────
def _load_drive_ids():
    return _ccu.load_drive_ids("madihue")


def _save_drive_ids(ids):
    _ccu.save_drive_ids("madihue", ids)


def actualizar_drive(drive_svc, outdir):
    return _ccu.actualizar_drive_organizado(drive_svc, outdir, CHART_NAMES, OBRA_FOLDER, "madihue")
...
def insertar_imagenes_en_sheets(sheets_svc, drive_ids):
    log.info("Actualizando formulas IMAGE() en hoja 'Curvas S' Com. Madihue...")
    meta = sheets_svc.spreadsheets().get(spreadsheetId=SPREADSHEET_ID).execute()
    curvas_gid = None
    for s in meta["sheets"]:
    ...
    W_PX, H_PX = 755, 385
    layout = [
        ("CurvaS_GRUPO_1_Madihue.png",     0, 0),
        ("CurvaS_TOTAL_Madihue.png",        1, 0),
        ("CurvaS_Todos_Grupos_Madihue.png", 0, 1),
    ]
...
def main():
    ...
    log.info("=" * 65)
    log.info(f"INICIO CURVAS S COM. MADIHUE  -  {ts}")
    log.info("=" * 65)
    try:
        ...
        log.info("=" * 65)
        log.info("COMPLETADO. Curvas S Com. Madihue actualizadas.")
        log.info("=" * 65)
