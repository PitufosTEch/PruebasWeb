...
| 2026-05-19 | **Corrección de la fuente:** la hoja correcta NO se llama "Seguimiento Cierre de Obras" sino `documentacion` (lowercase) en el mismo spreadsheet `1JAxxP9W...0Zk`. Las variantes "Seguimiento Cierre…", "SeguimientoCierre", "Seguimiento_Cierre" devolvían "Hoja no encontrada". `documentacion` agregada al fetch y como primer candidato en SEG_TABLE_CANDIDATES. La hoja "Seguimiento" queda como fallback legacy. | Claude (confirmado por el usuario tras inspeccionar el Sheet) |
| 2026-05-19 | **Fuente DUAL:** el usuario confirma que la info completa de cierre está repartida entre `documentacion` (flags de existencia de documento) y `Beneficiario` columnas **BT en adelante** (datos reales, fechas, valores). Vista AppSheet asociada: "Centro de cierre documentaciones". El procesamiento ahora: (1) levanta `SEGUIMIENTO_DATA` desde `documentacion`, (2) enriquece desde `Beneficiario[col BT+]` (0-indexed: idx 71+), rellenando solo los campos vacíos. Si un campo tiene valor en `documentacion`, gana esa fuente. Debug panel muestra cuántas columnas BT+ se usaron y qué columna matcheó cada checkpoint en esa fuente secundaria. | Claude (confirmado por el usuario) |
| 2026-05-19 | **Precedencia BT+ sobre documentacion + fallback Acta F1:** se invierte el merge. Ahora `Beneficiario.BT+` GANA si tiene valor (es el dato real: fecha/Si). `documentacion` queda como fallback. Caso de uso del usuario: Newen Ruka tiene la Acta F1 subida en `documentacion.F1` (PDF) pero `Beneficiario.F_F1` (fecha) está vacío — antes el checkpoint F1 se mostraba apagado, ahora se prende usando la existencia del PDF como evidencia. Se agregó regla `{must:['f1'], not:[…]}` a `fecha_f1` para detectar la columna `F1` de documentacion. En la vista expandida, valores que son rutas de PDF se muestran como "✓ Doc cargado" con tooltip explicativo. | Claude (a pedido del usuario) |
