...
def get_data_loader_js(apps_script_url):
    ...
    const CACHE_KEY = 'scraices_data_cache_v3';
    const CACHE_MAX_AGE = 4 * 60 * 60 * 1000; // 4 horas

    function saveCache(raw) {{
        try {{
            const payload = {{ ts: Date.now(), data: raw }};
            localStorage.setItem(CACHE_KEY, JSON.stringify(payload));
            console.log('[CACHE] Guardado', Math.round(JSON.stringify(payload).length / 1024), 'KB');
        }} catch (e) {{
            console.warn('[CACHE] No se pudo guardar:', e.message);
    ...
    function loadCache() {{
        try {{
            ...
            console.log('[CACHE] Cargado, edad:', Math.round(age / 60000), 'min');
            return payload;
        }} catch (e) {{
    ...
    function showCacheBanner(ts) {{
        ...
        banner.style.cssText = 'position:fixed;top:0;left:0;right:0;z-index:9999;background:#f59e0b;color:#78350f;text-align:center;padding:6px 16px;font-size:12px;font-family:IBM Plex Sans,sans-serif;display:flex;align-items:center;justify-content:center;gap:8px;';
    ...
    async function initLiveData() {{
        ...
        if (cached) {{
            try {{
                processRawData(cached.data);
                renderApp();
                showCacheBanner(cached.ts);
                usedCache = true;
            }} catch (e) {{
                console.warn('[CACHE] Datos invalidos, descargando frescos');
            }}
        }}

        if (!usedCache) showLoadingScreen();

        // 2. Fetch datos frescos (en paralelo si hay cache, o como carga principal)
        try {{
            ...
            if (usedCache) {{
                // Ya tenemos datos de cache, solo avisar que no se pudo actualizar
                const b = document.getElementById('cacheBanner');
                if (b) {{
                    b.style.background = '#fecaca';
                    b.style.color = '#991b1b';
                    b.innerHTML = `<span>&#9888; No se pudo actualizar (${{err.message}}). Usando datos en cache (${{formatCacheAge(cached.ts)}})</span><button onclick="location.reload()" style="margin-left:12px;padding:3px 12px;background:#991b1b;color:white;border:none;border-radius:4px;cursor:pointer;font-size:11px;">Reintentar</button>`;
                }}
            }} else {{
                document.getElementById('root').innerHTML = `
                    <div style="min-height:100vh;display:flex;align-items:center;justify-content:center;background:#f3f4f6;font-family:'IBM Plex Sans',sans-serif;">
